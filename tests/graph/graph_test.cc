...
TEST_CASE("Graph graph", "[graph]") {
  ...
  SECTION("Test graph creating") {
    ...
    SECTION("Create an edge") {
      ...
      SECTION("Create undirected edges") {
        ...
        // Create edge
        REQUIRE(graph->create_edge(0, 1, false) == true);
        REQUIRE(graph->nedges() == 2);

        // Non-existant edge
        REQUIRE(graph->create_edge(2, 1, false) == false);
        REQUIRE(graph->nedges() == 2);
      }
    }
  }
  SECTION("Create test graph") {
    ...
    // Create edges
    REQUIRE(graph->create_edge(0, 1, true) == true);  // ID 0
    REQUIRE(graph->create_edge(0, 2, true) == true);  // ID 1
    REQUIRE(graph->create_edge(0, 5, true) == true);  // ID 2
    REQUIRE(graph->create_edge(1, 2, true) == true);  // ID 3
    REQUIRE(graph->create_edge(1, 3, true) == true);  // ID 4
    REQUIRE(graph->create_edge(2, 0, true) == true);  // ID 5
    REQUIRE(graph->create_edge(2, 3, true) == true);  // ID 6
    REQUIRE(graph->create_edge(2, 5, true) == true);  // ID 7
    REQUIRE(graph->create_edge(3, 2, true) == true);  // ID 8
    REQUIRE(graph->create_edge(3, 4, true) == true);  // ID 9
    REQUIRE(graph->create_edge(4, 5, true) == true);  // ID 10
