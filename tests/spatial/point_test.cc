...
TEST_CASE("Spatial point", "[spatial][point]") {
  ...
  SECTION("Test point") {
    ...
    auto point = std::make_shared<cityscape::spatial::Point>(nid);
    // Test point id
    REQUIRE(point->id() == 1);

    // Check coordinates
    point->coordinate(coordinates[0], coordinates[1]);

    auto coord = point->coordinate();

    REQUIRE(coord[0] == Approx(coordinates.at(0)).epsilon(Tolerance));
    REQUIRE(coord[1] == Approx(coordinates.at(1)).epsilon(Tolerance));
  }

  SECTION("Test Point Comparison") {
    std::array<double, 2> coords1 = {1., 1.};
    cityscape::spatial::Point p1{1};
    p1.coordinate(coords1[0], coords1[1]);

    std::array<double, 2> coords2 = {2., 2.};
    cityscape::spatial::Point p2(2, coords2);

    cityscape::spatial::Point p3(3);
    p3.coordinate(coords2[0], coords2[1]);
  ...
  SECTION("Test Boost Geometry distances") {
    // Coordinates for 2d
    std::array<double, 2> coords1 = {1., 1.};
    cityscape::spatial::Point p1(1, coords1);
  ...
  //    auto point =
  //        std::make_shared<cityscape::spatial::Point<2>>(nid, name,
  //        coordinates);
  //    // Test point id
  ...
  //    cityscape::spatial::Point<2, bg::cs::geographic<bg ::degree>> p1{1,
  //    "s1",
  //                                                                     coords1};
  //
  //    std::array<double, 2> coords2 = {4.3826169, 50.8119483};
  //    cityscape::spatial::Point<2, bg::cs::geographic<bg ::degree>> p2{2,
  //    "s2",
  //                                                                     coords2};
  //
  //    REQUIRE(boost::geometry::distance(p1, p2) ==
